...
export default function ScriptsManager() {
  ...
  const fetchScripts = async () => {
    setLoading(true)
    const { data, error }_ = await supabase.from('scripts').select('*').order('created_at')
  ...
  const addScript = async (e) => {
    e.preventDefault()
    setError(null)
    const { error }_ = await supabase.from('scripts').insert({
      name: newScriptName,
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-8">
      <h1 className="text-2xl font-bold text-slate-900 mb-6">A/B Script Manager</h1>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        {/* Add New Script Form */}
        <div className="bg-white p-6 rounded-lg shadow-xl">
          <h2 className="text-xl font-semibold text-slate-800 mb-4">Add New Script</h2>
          <form onSubmit={addScript} className="space-y-4">
            <div>
              <label htmlFor="name" className="block text-sm font-medium text-slate-700">Script Name</label>
              <input
                id="name"
                type="text"
                value={newScriptName}
            ...
            <div>
              <label htmlFor="content" className="block text-sm font-medium text-slate-700">Script Content</label>
              <textarea
                id="content"
                rows="5"
        ...
        <div className="space-y-4">
          <h2 className="text-xl font-semibold text-slate-800 mb-4">Current Scripts</h2>
          {loading ? <p>Loading scripts...</p> : scripts.map(script => (
            <div key={script.id} className="bg-white p-4 rounded-lg shadow-md">
              ...
              <p className="text-sm text-slate-600 mt-2 p-3 bg-slate-50 rounded">{script.content}</p>
            </div>
          ))}
        </div>
      </div>
