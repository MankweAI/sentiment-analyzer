...
export default function CallSuitModal({ prospect, onClose, onLogSaved }) {
  ...
  useEffect(() => {
    // Correctly handle missing pain_data
    const painDataText =
      prospect.pain_data ||
      (prospect.leaks.find((l) => l.includes("Trust"))
        ? prospect.leaks.find((l) => l.includes("Trust")).split(": ")[1]
        : "low reviews");
    const competitorPainData = "their high review count"; // Placeholder
    ...
    setScripts({
      ...
      owner: `Ok great, I'll be quick. My name is Mankwe, and I've just done a competition report for Randburg plumbers. I noticed Google is showing your competitor, ${prospect.competitor}, to all new customers. I found it's because your website has ${painDataText}, while your competition has ${competitorPainData}. My job is to fix that. Are you the right person to discuss this with?`,
    });
  }, [prospect]);
  ...
  const handleSubmit = async (e) => {
    ...
    if (prospect.status !== newStatus) {
      ...
      if (prospectError) {
        setError(
          `Log saved, but failed to update prospect status: ${prospectError.message}`
        );
        setSubmitting(false);
        return;
      }
        ...
        <div className="flex h-[calc(90vh-65px)]">
          ...
          <div className="w-1/2 p-6 overflow-y-auto">
            ...
            >
              ...
              <div className="p-3 bg-cta-light border border-cta-dark rounded-md">
            ...
            >
              ...
              <p className="text-lg font-medium text-slate-700 p-3 bg-primary-light border border-primary-dark rounded-md">
