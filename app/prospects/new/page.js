...
// BUG FIX 1: Components moved outside the render function
const LeakDropdown = ({ name, label, value, onChange }) => (
...
const TextField = (
  { name, label, placeholder, value, onChange, required = false } // Default required to false
) => (
  <div>
    <label htmlFor={name} className="block text-sm font-medium text-slate-700">
      {label}
    ...
    <input
      ...
      value={value}
      onChange={onChange}
      required={required} // BUG FIX 2: 'required' attribute is now controlled and defaults to false
...
export default function AddNewProspect() {
  ...
  const handleSubmit = async (e) => {
    ...
    // Optional: Add manual validation here if needed
    ...
    // BUG FIX: Removed the trailing underscore `_` which caused a parsing error.
      ...
      <div className="bg-white p-6 rounded-lg shadow-xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <TextField
            ...
            required={true} // We can make *this* one required, our check handles it
          />
          <TextField
            name="website"
            label="Website URL"
            placeholder="e.g., apexplumbing.co.za"
          ...
          <TextField
            ...
            placeholder="e.g., Northcliff, Randburg"
          ...
          <TextField
            ...
            placeholder="e.g., 4.1 stars vs 5.0"
            value={formData.pain_data}
            onChange={handleChange}
          />

          <h3 className="text-lg font-bold border-b pb-2 pt-4">
            Leak Analysis (The Proof)
          </h3>
          <div className="grid grid-cols-3 gap-4">
            <LeakDropdown
              name="trafficLeak"
              label="Traffic Leak (SEO)"
              value={formData.trafficLeak}
              onChange={handleChange}
            />
            <LeakDropdown
              name="trustLeak"
              label="Trust Leak (Reviews)"
              value={formData.trustLeak}
              onChange={handleChange}
            />
            <LeakDropdown
              name="enquiryLeak"
              label="Enquiry Leak (Capture)"
              value={formData.enquiryLeak}
              onChange={handleChange}
            />
          </div>
